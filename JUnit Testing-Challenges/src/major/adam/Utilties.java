...
public class Utilties {
    public char[] everyNthChar(char[] sourceArray, int n) {
        ...
        if (sourceArray == null || sourceArray.length == 0) throw new IllegalArgumentException("sourceArray must be at least length 1");
        if (sourceArray.length < n) throw new IllegalArgumentException("sourceArray doesn't have n number of items");

        char[] toReturn = new char[(int) Math.ceil(sourceArray.length / n)];
        int toReturnIndex = 0;
